...
setup(function (_) {
  assert_implements_optional(document.createElement('link').relList.supports('prefetch'), "Browser supports prefetch.");
  assert_implements_optional("PerformanceResourceTiming" in window, "Browser supports performance APIs.");
});
...
function assert_link_does_not_prefetch(test, link) {
  ...
  waitUntilCSPEventForURL(test, link.href).then(test.step_func(function (e) {
    cspEvent = true;
    assert_equals(e.violatedDirective, "prefetch-src");
    assert_equals(e.effectiveDirective, "prefetch-src");
    if (errorEvent) test.done();
  }));
  link.onerror = test.step_func(function (e) {
    errorEvent = true;
    if (cspEvent) test.done();
  });
  link.onload = test.unreached_func('onload should not fire.');
  document.head.appendChild(link);
}
function try_to_prefetch(_x, _x2) {
  return _try_to_prefetch.apply(this, arguments);
}
function _try_to_prefetch() {
  _try_to_prefetch = _asyncToGenerator( /*#__PURE__*/_regeneratorRuntime().mark(function _callee(href, test) {
    var url, link, didPrefetch, entry;
    return _regeneratorRuntime().wrap(function _callee$(_context) {
      while (1) switch (_context.prev = _context.next) {
        case 0:
          ...
          didPrefetch = new Promise(function (resolve) {
            var observer = new PerformanceObserver(function (list) {
              var entries = list.getEntriesByName(link.href);
              if (entries.length) {
                resolve(entries[0]);
              }
            });
        ...
        case 12:
          entry = _context.sent;
          return _context.abrupt("return", entry.requestStart > 0 && entry.decodedBodySize > 0);
        case 14:
        case "end":
