...
function assert_no_csp_event_for_url(test, url) {
  self.addEventListener("securitypolicyviolation", test.step_func(function (e) {
    if (e.blockedURI !== url) return;
...
function waitUntilCSPEventForURLOrLine(test, url, line) {
  return new Promise(function (resolve, reject) {
    self.addEventListener("securitypolicyviolation", test.step_func(function (e) {
      if (e.blockedURI == url && (!line || line == e.lineNumber)) resolve(e);
    }));
  });
}
...
function assert_worker_is_loaded(url, description) {
  var expected_message = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : "ping";
  async_test(function (t) {
    assert_no_csp_event_for_url(t, url);
    var w = new Worker(url);
    assert_no_event(t, w, "error");
    waitUntilEvent(w, "message").then(t.step_func_done(function (e) {
...
function assert_shared_worker_is_loaded(url, description) {
  ...
  async_test(function (t) {
    ...
    assert_no_event(t, w, "error");
    waitUntilEvent(w.port, "message").then(t.step_func_done(function (e) {
      assert_equals(e.data, expected_message);
    }));
    w.port.start();
  }, description);
...
function assert_service_worker_is_loaded(url, description) {
  promise_test(function (t) {
    assert_no_csp_event_for_url(t, url);
    return Promise.all([waitUntilEvent(navigator.serviceWorker, "message").then(function (e) {
      assert_equals(e.data, "ping");
    }), navigator.serviceWorker.register(url, {
      scope: url
    }).then(function (r) {
      var sw = r.active || r.installing || r.waiting;
      t.add_cleanup(function (_) {
        return r.unregister();
      });
      sw.postMessage("pong?");
    })]);
...
function assert_worker_is_blocked(url, description) {
  async_test(function (t) {
    ...
    waitUntilCSPEventForURL(t, reportedURL).then(t.step_func_done(function (e) {
...
function assert_service_worker_is_blocked(url, description) {
  promise_test(function (t) {
    ...
    // If |url| is a blob, it will be stripped down to "blob" for reporting.
    var reportedURL = new URL(url).protocol == "blob:" ? "blob" : url;
    return Promise.all([waitUntilCSPEventForURL(t, reportedURL).then(t.step_func_done(function (e) {
      assert_equals(e.blockedURI, reportedURL);
      assert_equals(e.violatedDirective, "worker-src");
      assert_equals(e.effectiveDirective, "worker-src");
    })), promise_rejects_dom(t, "SecurityError", navigator.serviceWorker.register(url, {
      scope: url
    }))]);
  }, description);
}
