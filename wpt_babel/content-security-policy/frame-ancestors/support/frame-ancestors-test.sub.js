...
window.addEventListener("message", function (e) {
  if (window.parent != window) window.parent.postMessage(e.data, "*");else if (e.data.type === 'test_result') endTest(e.data.failed, "Inner IFrame msg: " + e.data.message);
});
function injectNestedIframe(policy, parent, child, expectation, isSandboxed) {
  var iframe = document.createElement("iframe");
  var url = "/content-security-policy/frame-ancestors/support/frame-in-frame.sub.html" + "?policy=" + policy + "&parent=" + parent + "&child=" + child + "&expectation=" + expectation;
  url = (parent == "same" ? SAMEORIGIN_ORIGIN : CROSSORIGIN_ORIGIN) + url;
  iframe.src = url;
  if (isSandboxed) iframe.sandbox = 'allow-scripts';
  document.body.appendChild(iframe);
}
var timer;
function pollForLoadCompletion(_ref) {
  var iframe = _ref.iframe,
    expectBlock = _ref.expectBlock;
  var fn = iframeLoaded({
    expectBlock: expectBlock,
    isPoll: true
  });
  timer = test.step_timeout(function () {
    return fn({
      target: iframe
    });
  }, 10);
...
function injectIFrame(policy, sameOrigin, expectBlock) {
  ...
  if (sameOrigin) url = SAMEORIGIN_ORIGIN + url;else url = CROSSORIGIN_ORIGIN + url;
  iframe.src = url;
  document.body.appendChild(iframe);
...
function iframeLoaded(_ref2) {
  var isPoll = _ref2.isPoll,
    expectBlock = _ref2.expectBlock;
  return function (ev) {
    clearTimeout(timer);
    var failed = true;
    var message = "";
    try {
      var url = ev.target.contentWindow.location.href;
