...
function runTest(navigationShouldAllowed, navigationMethod, description) {
  var t1 = async_test('javascript: navigation using ' + navigationMethod + ' should be ' + (navigationShouldAllowed ? 'allowed' : 'refused') + description);
  if (navigationShouldAllowed) {
    window.navigated = function () {
      return t1.done();
    };
    window.addEventListener('securitypolicyviolation', t1.unreached_func('Should have not raised any event'));
  } else {
    window.navigated = t1.unreached_func('Should not have run javascript: URL');
  ...
  if (navigationMethod === '<a href target=_blank>') {
    ...
    _a.setAttribute('href', 'javascript:navigated();');
